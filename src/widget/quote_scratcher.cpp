...
{
    if (!w.forceScale) {
        auto trade_it = std::lower_bound(mDataManager->PublicTradeCache().begin(), mDataManager->PublicTradeCache().end(),
            w.GetDataViewRect().x, [](const Trade& t, const uint64_t& x) {
        ...
        for(; trade_it != mDataManager->PublicTradeCache().end(); ++trade_it) {
...
void QuoteScratcher::Paint(DataScratchWidget &w) const
{
    ...
    auto trade_it = std::lower_bound(mDataManager->PublicTradeCache().begin(), mDataManager->PublicTradeCache().end(),
        w.GetDataViewRect().x, [](const auto& t, const auto& x) {
            return duration_cast<milliseconds>(t.trade_time.time_since_epoch()).count() >= x;
        });
